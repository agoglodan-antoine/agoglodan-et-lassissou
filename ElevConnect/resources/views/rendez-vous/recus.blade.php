                ...
                <td>
                  <div class="dash-actions">
                    @if ($rdv->statut === 'en_attente')
                      <form method="POST" action="{{ route('rendez-vous-recus.confirmer', $rdv) }}">
                        @csrf
                        <button type="submit" class="btn btn-primary btn-sm">Confirmer</button>
                      </form>
                      <form method="POST" action="{{ route('rendez-vous-recus.refuser', $rdv) }}">
                        @csrf
                        <button type="submit" class="btn btn-danger btn-sm">Refuser</button>
                      </form>
                    @endif
                    @if ($rdv->statut === 'confirme')
                      <form method="POST" action="{{ route('rendez-vous-recus.realise', $rdv) }}">
                        @csrf
