  ...
  <div class="container">
    ...
    <div class="pricing-grid" style="margin-bottom:32px;">
      <div class="price-card">
        ...
        @if (! $estPremium)
          <span class="status-pill visible" style="margin-top:20px;display:inline-block;">Formule actuelle</span>
        @else
          <form method="POST" action="{{ route('abonnement.souscrire') }}" style="margin-top:20px;">
            @csrf
          ...
          <tbody>
            @foreach ($historique as $ab)
              <tr>
                ...
                <td><span class="status-pill {{ $ab->statut === 'actif' ? 'visible' : 'rejetee' }}">{{ $ab->statut }}</span></td>
              </tr>
            @endforeach
          </tbody>
        </table>
