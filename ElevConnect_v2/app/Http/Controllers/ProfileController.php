...
class ProfileController extends Controller
{
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('photo_profil')) {
            ...
            try {
                $utilisateurData['photo_profil'] = $request->file('photo_profil')->store('profils', 'public');
            } catch (\Throwable $e) {
                return back()->withErrors([
                    'photo_profil' => "La photo n'a pas pu être enregistrée sur le serveur ({$e->getMessage()}).",
                ]);
            }
        }
