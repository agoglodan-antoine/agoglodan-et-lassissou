...
- **Code QR** : généré avec le paquet `simplesoftwareio/simple-qrcode`
  ...
  SVG pur PHP), affiché sur la page « Mes achats » dès que la commande est
  payée.
...
cd elevconnect
# copier par-dessus : app/, bootstrap/app.php, config/elevconnect.php,
# database/migrations/, resources/, routes/, public/css/ (dashboard.css et
# auth.css uniquement — app.css est désormais dans resources/css/), .env.example,
...
npm install
npm run build   # ou "npm run dev" pendant le développement (rechargement à chaud)

php artisan serve
```

**CSS/JS communs à tout le site (design system, header/footer publics,
chargement progressif au défilement)** : centralisés dans `resources/css/app.css`
et `resources/js/app.js`, compilés par Vite (`vite.config.js`) et injectés dans
les deux layouts via `@vite([...])`. Les réglages de timing de l'animation
d'apparition au défilement (durée, délai, effet de cascade) sont commentés
directement dans ces deux fichiers, avec des variables/constantes nommées à
ajuster librement (`--reveal-duration`, `--reveal-base-delay` côté CSS ;
`REVEAL_STAGGER_MULTIPLIER`, `REVEAL_THRESHOLD`, `REVEAL_ROOT_MARGIN` côté JS).
`public/css/dashboard.css` et `public/css/auth.css` restent, eux, de simples
fichiers statiques chargés à la demande via `@push('styles')`.

**Middleware de rôle** : `App\Http\Middleware\EnsureRole` est enregistré dans
`bootstrap/app.php` et appliqué sur les routes concernées (`role:administrateur`,
`role:veterinaire`, etc. — voir `routes/web.php`).
