...
<section class="dash-section">
  <div class="container">
    ...
    <form method="GET" action="{{ route('mon-espace.admin.utilisateurs.index') }}" class="filter-bar">
        ...
        <tbody>
          @foreach ($utilisateurs as $u)
            <tr>
              ...
              <td>{{ $u->email }}</td>
              <td><x-status-pill :status="$u->statut" /></td>
              <td>{{ $u->date_inscription->format('d/m/Y') }}</td>
              <td>
                <div class="dash-actions">
                  <a href="{{ route('mon-espace.admin.utilisateurs.show', $u) }}" class="btn btn-ghost-dark btn-sm">Voir</a>
                  @if ($u->role !== 'administrateur')
                    @if ($u->statut === 'actif')
                      <form method="POST" action="{{ route('mon-espace.admin.utilisateurs.suspendre', $u) }}" onsubmit="return confirm('Suspendre ce compte ?');">
                        @csrf
                        <button type="submit" class="btn btn-danger btn-sm">Suspendre</button>
                      </form>
                    @else
                      <form method="POST" action="{{ route('mon-espace.admin.utilisateurs.reactiver', $u) }}">
                        @csrf
