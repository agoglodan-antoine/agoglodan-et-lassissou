  ...
  <div class="container">
    ...
    <form class="filter-bar" method="GET" action="{{ route('veterinaires.index') }}" id="filterForm">
      ...
      <button type="button" class="btn btn-ghost-dark" id="useLocationBtn">
        {{ $geolocalise ? '📍 Position activée' : '📍 Autour de moi' }}
      </button>
        ...
        @foreach ($veterinaires as $vet)
          <a href="{{ route('veterinaires.show', $vet->id_utilisateur) }}" style="text-decoration:none;color:inherit;">
            <div class="catalogue-card">
              <div class="body">
                @if ($vet->est_premium)
                  <span class="status-pill visible" style="margin-bottom:8px;">Premium</span>
                @endif
                ...
                @if ($vet->note_moyenne)
                  <div class="price">★ {{ number_format($vet->note_moyenne, 1) }} ({{ $vet->nombre_avis }} avis)</div>
                @else
