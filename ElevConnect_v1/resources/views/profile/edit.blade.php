...
<section class="dash-section">
  <div class="container">
    <a href="{{ route('mon-espace.dashboard') }}" class="me-back">
    ...
    <div class="auth-card" style="max-width:640px;">
      <form method="POST" action="{{ route('mon-espace.profil.update') }}">
        @csrf @method('PUT')
        ...
        <div class="geoloc-status {{ $user->latitude ? 'ok' : '' }}" id="geolocStatus" style="margin-top:6px;">
          @if ($user->latitude && $user->longitude)
            Position actuelle enregistrée ✓ — utilisée pour la recherche par proximité.
          @else
    ...
    <div class="auth-card" style="max-width:640px;margin-top:24px;">
      ...
      <form method="POST" action="{{ route('mon-espace.profil.password') }}">
        @csrf @method('PUT')
        <div class="form-field">
...
(function(){
  ...
  geolocBtn.addEventListener('click', function(){
    ...
    navigator.geolocation.getCurrentPosition(function(pos){
      ...
      geolocStatus.textContent = "Nouvelle position captée ✓ — enregistrez pour la sauvegarder.";
